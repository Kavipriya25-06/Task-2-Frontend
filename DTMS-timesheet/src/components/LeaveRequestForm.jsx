...
const LeaveRequestForm = ({ leaveType, onClose }) => {
  ...
  //   data.append("leave_type", mappedLeaveType);
  //   data.append("start_date", format(formData.startDate, "yyyy-MM-dd"));
  //   data.append("end_date", format(formData.endDate, "yyyy-MM-dd")); // Check here Meena
  //   data.append("duration", String(formData.duration)); // Check here Meena 
  //   data.append("reason", formData.reason);
  //   data.append(
  //     "resumption_date",
  //     format(formData.resumptionDate, "yyyy-MM-dd")
  ...
  //       await patchLeaveAvailability(mappedLeaveType, formData.duration); // Check here Meena
  //       await addAttachment(result.data.leave_taken_id);
...
const handleSubmit = async (e) => {
  ...
  if (!formData.startDate) return bail("Enter the Start date.");
  if (!formData.endDate) return bail("Enter the End date.");
  if (!formData.resumptionDate) return bail("Enter the Resumption date.");
  if (formData.endDate < formData.startDate) return bail("End date should be after start date.");
  if (!formData.duration) return bail("Enter the Duration.");
  if (formData.resumptionDate < formData.endDate) return bail("Enter the Resumption date.");
  ...
  // ---------- FIXED CL → LOP LOGIC ----------
  let clToApply = parseFloat(formData.duration);
  let lopDays = 0;

  if (mappedLeaveType === "casual_leave") {
    const availableCL = parseFloat(leaveSummary.casual_leave || 0);

    if (clToApply > availableCL) {
      lopDays = clToApply - availableCL;
      clToApply = availableCL;

      const confirmLop = window.confirm(
        `Your CL balance is ${availableCL}. The extra ${lopDays} day(s) will be applied as LOP. Do you want to proceed?`
      );

      if (!confirmLop) {
        return bail("Leave submission cancelled by user.");
      }
    }
  }

  // Submit the leave with CL only
  const apiURL = `${config.apiBaseURL}/leaves-taken/`;
  const data = new FormData();
  ...
  data.append("start_date", format(formData.startDate, "yyyy-MM-dd"));
  data.append("end_date", format(formData.endDate, "yyyy-MM-dd"));
  data.append("duration", String(clToApply)); // <-- use clToApply
  data.append("reason", formData.reason);
  ...
  data.append("employee", user.employee_id);
  ...
  try {
    ...
    if (response.ok) {
      ...
      // Update CL balance
      await patchLeaveAvailability(mappedLeaveType, clToApply);

      // If any LOP days, patch them separately
      if (lopDays > 0) {
        await patchLeaveAvailability("lop", lopDays);
      }
      ...
      if (newAttachments.length > 0) {
        for (const file of newAttachments) {
          ...
          formDataFile.append("leavestaken", result.data.leave_taken_id);
  ...
  // After successful CL submission
if (lopDays > 0) {
  const lopData = new FormData();
  lopData.append("leave_type", "lop");
  lopData.append("start_date", format(formData.startDate, "yyyy-MM-dd"));
  lopData.append("end_date", format(
      new Date(formData.startDate.getTime() + (lopDays - 1) * 86400000),
      "yyyy-MM-dd"
  ));
  lopData.append("duration", String(lopDays));
  lopData.append("reason", `Excess Casual Leave applied as LOP`);
  lopData.append("resumption_date", format(formData.resumptionDate, "yyyy-MM-dd"));
  lopData.append("employee", user.employee_id);
  lopData.append("status", "pending");

  await fetch(`${config.apiBaseURL}/leaves-taken/`, { method: "POST", body: lopData });
}
  ...
  return (
    <div className="form-containers">
      ...
      >
        ...
        <div className="row1">
          <div className="form-group-half1">
            <label className="label1">Durationnn</label>
